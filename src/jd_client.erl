...
-spec start(ListenSocket) -> Result
    when ListenSocket :: gen_tcp:socket(),
         ...
         Reason       :: {already_started, pid()}
                       ...
                       | term().
%% @private
%% How the jd_client_man or a prior jd_client kicks things off.
%% This is called in the context of jd_client_man or the prior jd_client.

start(ListenSocket) ->
    jd_client_sup:start_acceptor(ListenSocket).
...
-spec start_link(ListenSocket) -> Result
    when ListenSocket :: gen_tcp:socket(),
         ...
         Reason       :: {already_started, pid()}
                       ...
                       | term().
%% @private
%% This is called by the jd_client_sup. While start/1 is called to iniate a startup
...
%% the jd_client_man (so it can still close it on a call to jd_client_man:ignore/0),
